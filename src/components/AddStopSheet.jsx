...
import { QUICK_PICKS, CATEGORY_META } from "../utils/classify";
...
const AddStopSheet = ({ pendingCoord, onStartPin, onAdd, onClose }) => {
  ...
  const commit = () => {
    ...
    onAdd({
      ...
      nameHe: name.trim(),
      category: meta.he,
      coordinates: pendingCoord || null,
      _customPin: !!pendingCoord,
    });
  };
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 60, display: "flex", alignItems: "flex-end" }}>
      ...
      <div dir="rtl" style={{
        ...
        <button onClick={onStartPin}
          style={{ width: "100%", height: 46, borderRadius: 14, border: "1px dashed rgba(20,20,20,0.2)", background: pendingCoord ? "#E4EFE5" : "transparent", color: "#2A3036", fontSize: 13.5, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", marginBottom: 16 }}>
          {pendingCoord ? `📍 מיקום נבחר (${pendingCoord.lat.toFixed(4)}, ${pendingCoord.lng.toFixed(4)})` : "נעצו סיכה ידנית על המפה"}
        </button>
